...
def analyze_and_persist(
    ...
    try:
        topology, risk_scores, attack_paths, gnri = analysis_service.run_core_analysis(request)
                ...
                PathAnalysisSchema(
                    ...
                    hops=[
                        HopDetailSchema(
                            ...
                            cves=hop.get("cves", []),
                            etp=hop.get("etp", 0.0),
                            edge_cvss=hop.get("edge_cvss"),
                        )
                        for hop in path.hop_details
                    ],
                )
...
@router.get("/paths/{snapshot_id}", response_model=list[PathAnalysisSchema])
def get_snapshot_paths(
    snapshot_id: int,
    db: Session = Depends(get_db),
    persistence_service: PersistenceService = Depends(get_persistence_service),
    current_user: User = Depends(require_viewer_or_above),
) -> list[PathAnalysisSchema]:
    """Return the saved attack paths for a snapshot, with per-hop CVE annotations."""
    snapshot = persistence_service.get_snapshot_results(db, snapshot_id)
    if snapshot is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Snapshot not found")
    ...
    return [
        PathAnalysisSchema(
            ...
            hops=[
                ...
                for hop in (record.path_data.get("hops", []) if record.path_data else [])
            ],
        )
        for record in snapshot.attack_paths
    ]
